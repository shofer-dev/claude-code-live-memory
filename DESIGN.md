...
- **Two kinds of evictable content, handled differently:**
  - **File contexts** are *re-readable from disk* (the file is the source of truth) → **evict (LRU) + lazy-reload** on next reference. No knowledge lost.
...
**b. Task/tool edits** — `PostToolUse` hook (matcher `Write|Edit`) → HTTP POST → **filter: if `path` is in the loaded-files set**, add to `recentlyModifiedFiles` (else drop — a file Live Memory never read has no prior knowledge to be stale). **No eviction** (append-only preserved); the set is drained into the next question's trailing-turn hint.

Both file-change paths are thus scoped to **files Live Memory has actually read** — and that read set grows as it answers more questions.
