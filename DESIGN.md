...
- The `ask_live_memory` MCP tool is the uniform, provider-independent **query** contract; args `question`, `cwd`, mandatory soft `timeout`, and an optional `max_answer_tokens`.
- **Answer-length budget is enforced *and* disclosed.** The model's generation is hard-capped (`chat(max_tokens=…)`, default `LIVE_MEMORY_MAX_ANSWER_TOKENS`); an undisclosed cap silently truncates a long answer mid-sentence. So the resolved budget is fed back to the model in the per-question hints (like the soft time budget), letting it self-regulate — leading with the most important facts and giving a prioritized summary rather than overrunning the cap. The caller overrides it per question via `max_answer_tokens` (0/absent → the server default; clamped to `[MIN, MAX]_ANSWER_TOKENS`): raise it for a deliberately long, detailed answer, lower it to force a terse one.
